...
We have discussed consistency as an all-or-nothing proposition: a
distributed applications provides strong consistency or it does
not. In many real-world scenarios, it makes sense to work with data
that is consistent up to some $\epsilon > 0$. For instance, it may be
...
tolerable level, we can imagine applications that provide relatively
good consistency and availability, without offering either perfect C
or A according to the CAP theorem. By controlling the threshold of
acceptable inconsistency as a continuous quantity, one can even
imagine precisely controlling the observable tradeoff between
consistency and performance, allowing application developers to trade
one for the other in a gradual fashion.

Yu and Vahdat explored the CAP tradeoff from this perspective in a
series of papers. Based on their idea of \emph{conits}, a unit of
continuous consistency. They built a prototype toolkit called TACT,
which allows applications to specify precisely their desired levels of
consistency.
\begin{enumerate}
  \item 2000 Building Replicated Systems TACT
  \item 2000 Efficient Numerical Error
  \item 2000 Design and Evaluation of a Conit Model
  \item 2001 Combining Generality Practicality
  \item 2002 Design and Eval of a Conit-based Model
\end{enumerate}

An interesting aspect of this work is that consistency can be tuned
\emph{dynamically}. This is desirable because one does not know \emph{a priori}
how much consistency or availability is acceptable. Indeed, the ideal
tradeoff will depend on the precise network performance and
...
In the conit framework, each conit is associated with three
...
\subsubsection*{Numeric consistency}
