...
Figures \ref{fig:smEx0} and \ref{fig:smEx1} depict time diagrams for
the shared memory abstraction, not unlike the diagrams in Figure
...
\begin{figure}[h]
  ...
  \caption{Concurrent updates by distributed nodes}
  \label{fig:smEx2}
\end{figure}

\begin{example}
  \label{exmpl:concurrentupdates}
  Examine Figure \ref{fig:smEx2}. Two writes are depicted that overlap
...
conform to the physical order of events, made precise below in the
form of ``external order.'' Indeed, below we show how sequential
consistency (Definition \ref{def:sequentiallyconsistent}), one of two
``strong'' models, allows for this possibility. Yet even this model
imposes too many constraints to be workable for networking
...
\begin{figure}[p]
  ...
  \begin{subfigure}{1\textwidth}
    \centering \input{images/pgf/smEx1DAG.pgf}
...
\begin{example}
  Figure \ref{fig:external1} shows a time diagram involving three
  processes.  Figure \ref{fig:externalDAG} depicts the external order
  relation among the operations in the form of a directed acyclic
  graph (DAG).
\end{example}
...
in terms of \emph{linearizations.}
...
\begin{figure}[p]
  ...
  \caption{Linearizations of Figure \ref{fig:smEx2}, with possible linearization points}
  \label{fig:smEx3}
\end{figure}
...
\begin{figure}[p]
  ...
  \begin{subfigure}{1\textwidth}
    \centering
    \input{images/pgf/linearEx1.pgf}
    \caption{Linearizable execution}
    \label{fig:linearEx1}
  \end{subfigure}

  \begin{subfigure}{1\textwidth}
    \input{images/pgf/linearEx11.pgf}
    \caption{One possible linearization}
    \label{fig:linearEx11}
  \end{subfigure}

  \begin{subfigure}{1\textwidth}
    \input{images/pgf/linearEx12.pgf}
...
So far, linearizability is a condition on individual executions of a
