...
\begin{figure}
  ...
  \begin{subfigure}{1\textwidth}
    \centering
    \includegraphics[width=0.6\textwidth]{images/figure\_placeholder.png}
...
% When $P$ receives messages during an anti-entropy session with $Q$,
% $Q$ it updates its summary vector to the maximum timestamp of any
% message it has received from $Q$. Because $Q$ always sends at least
% the messages originating at $Q$ and not yet received by $P$, the
% timestamp of the largest message completely summarizes which
% messages originated by $Q$ are known to $P$.
...
% For the same reason, timestamps using a conventional logical scalar
% clock also cannot be used, since there is no bound on how far apart
% two scalar clocks may diverge at any moment.
...
\begin{comment}
we assume that messages are delivered by TSAE in a total order,
forming a linear chain of delivered messages. We say that the
delivered messages are \emph{committed}. When a message is sitting in
the message buffer, received but not delivered by the ordering
component, we say it is \emph{uncommitted}. Recalling that the message
log accumulates linearly, this means each replica has a linear chain
of messages, some of which are committed and some of which are
not. Thus, the committed messages of any two replicas are the same or
one is a prefix of the other, but messages in their uncommitted parts
may be different, and uncommitted messages in both lots are generally
in different orders.
\end{comment}
...
\begin{definition}[Ideal image]
  ...
  % particular time $t$, the state that all
  % replicas would eventually converge to if further updates stop and
  % all processes are able to communicate. $\WL{\mathsf{ideal}}$
\end{definition}
