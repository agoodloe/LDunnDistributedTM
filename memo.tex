...
guarantees of strong consistency models with the flexibility and
...
quantitatively, and then to bound. In the limit, imposing tighter
bounds on inconsistency eventually comes at the cost of system
performance. The advantage of the conit model is that the precise
tradeoff can configured according to policies set by the application,
rather than being determined directly by the consistency
model. Indeed, it is possible to adjust these policies at runtime,
perhaps in response to factors like application workload and network
capacity, which might provide the precise sort of behavioral
continuity foreshadowed in Section
\ref{ssec:communication-and-safety}.
...
replica of the database. For simplicity we assume that the set
$\AllProc$ of participating processes is fixed and known to each
member, though one can imagine extending the framework to support
dynamic group membership following Golding
\cite{1992:golding-thesis}.
...
$D_P$. $D_P$ changes as $P$ learns about updates to the data, which
...
of request a data \emph{access}. Unlike in Section
\ref{ssec:shared-memory}, we allow for write accesses that are more
complex than simple instructions to update the value of a logical
...
steps that can modify several data items and might also return a value
to the user. In fact, read accesses should be thought of as merely
special cases of write accesses that do not modify any data. Using
terminology we will introduce later, a read access is effectively one
that applies zero numerical and order error to every conit, so it does
not need to be entered into $\WL{P}$ or participate in write
propagation.
...
$D + r = D$ if $r$ is a read. In many real-world applications, writes
accesses are not commutative, meaning in general we have the following
inequality:
\[
D + w + w' \neq D + w' + w
\]
...
to optimistically deviate from the total order within some
controllable bounds, which can improve application performance.
...
written $\Dstate{P}$, is governed by the equation
\begin{equation}
  \Dstate{P} = \Dinit + \localhistory{P}. \label{eq:conit-D-obs}
\end{equation}

If $H$ is any history, for each access $a \in H$ in the history, the
\emph{prefix} history of $a$, written $\PH{\left(a\right)}$, is the
set of accesses in $H$ that precede $a$. The prefix history determines
the value of $D_P$ ``seen'' by $a$. Note that if $a$ is a write
access, which can contain business logic, the observed state of $D_P$
...
\begin{definition}
  ...
  access $a$ originating at $P$ is the prefix of the local history
  $\localhistory{P}$ at the time $a$ is applied. The \emph{observed
    state} of an access $a$ is the state of $\Dstate{P}$ at the time
  $a$ is applied.
  \begin{gather}
...
access observes a value of $D_P$ whose divergence from its correct or
``ideal'' value is limited by an upper bound. The ideal value of $D_P$
...
determines their total order. This is an I/O-bound operation,
described in Section~\ref{ssec:conit-order-consistency}, since it
generally requires pulling updates from other processes over the
network.
...
maintains a write log $\WL{P}$, which is used as the message log of
TSAE. $P$ also maintains the other data structures required by TSAE,
whether in the form of summary and acknowledgment vectors (Sections
\ref{ssec:tsae-summary-vectors} and~\ref{ssec:tsae-acknowledgment})
or summary matrices (Section~\ref{ssec:tsae-unsynchronized}).
...
they are applied to $D_P$. Remote write access added to $\WL{P}$
during anti-entropy are applied to $D_P$ in the order they are
received.  Thus, at all times $\WL{P}$ contains the history of all
updates applied to $D_P$ and we have the following equation:
\begin{equation}
...
determine which writes it may need to send them, and likewise sends it
summary vector to others to receive updates from them. Messages are
only ever sent in batches determined by comparing summary vectors, and
properties like the coverage property (Section
...
\end{definition}
\noindent The necessity of Definition
\ref{def:conit-causal-precedence} is seen by imagining that $A$ and
$A'$ represent long-running isolated database transactions executed
currently. In this case, one operation will logically take effect (and
be entered into the write log) before the other, which is a causal
relation, but the operations have no external order relation.
...
\begin{definition}
  ...
  \end{equation}
\end{definition}
An ECG history is a causally consistent linearization of the global
system history. We will refer to an ECG history as simply a ``global''
history---the fact this history must respect external and causal order
is implicit. Relative to a global history, the ideal database image is
the state it would be in after applying all the updates in the global
history in order.
\begin{definition}
  If $\Hglobal$ is given as a global history, the ideal database image
...
values. This simple sort of data model was used in Section
...
database, we take the opposite approach: the application programmer is
entirely responsible for informing the middleware how much change each
update applies to each conit. These values are not enforced by the
middleware, but simply accepted as ground truths as used as inputs for
...
suppose $P$ and $Q$ are initialized in state $\Dinit$. Suppose a
client submits two writes accesses $w$ and $w'$ to $P$ with the following weights:~\\

\begin{minipage}{0.5\textwidth}
\begin{itemize}
\item[] $\NumWeight(w, F_y) = 2$
\item[] $\NumWeight(w, F_z) = 5$
\end{itemize}
\end{minipage}%
\begin{minipage}{0.5\textwidth}
  \begin{itemize}
  \item[] $\NumWeight(w', F_y) = 1$
  \item[] $\NumWeight(w', F_z) = -2$
  \end{itemize}
\end{minipage}~\\

\noindent If $Q$ has not seen $w$ and $w'$ yet, then the numerical
error of $D_Q$ with respect to $F_y$ and $F_z$ is $2 + 1 = 3$ and
$5 - 2 = 3$, respectively. The value $\NE\left(P, F\right)$ sets a
bound on the total numerical weight applied to $F$ of writes
that $P$ is unaware of---setting this value to $0$ means $P$ will be
notified of every write access that affects $F$ originating at any
process before that write request is finished executing. This process
will be described in Section~\ref{ssec:conit-numerical-consistency}.

Separately form numerical weight, each write is associated with an
...
Vahdat \cite{2002tact} has the programmer to invoke function calls of
the form $\mathsf{AffectConit}(F, x, y)$ inside in the body of
functions that modify the backing data store. Here, $F$ is the conit,
$x$ is $\NumWeight(w, F)$, and $y$ is $\OrderWeight(w, F)$.
...
mechanisms. Real-time staleness will is described in Section
\ref{ssec:conit-real-time-consistency}.

\subsubsection{Enforcing Error Bounds}
\label{sssec:conit-tuning-bounds}
%TODO Compulsory anti-entropy
...
%TODO
Explain when conits can be changed.
...
not exceed an allotted ``share'' of $Q$'s total error bound, or
...
$P$, all of the writes with negative weight. If $P$ totaled all
...
\begin{example}
  ...
  their reservation has been canceled, alongside issuing a refund (and
...
in an ideal history, where the set of writes preceding it constitutes
...
the order error of an access by totaling the order weight of accesses
...
the $P$ engages the write commitment procedure assumed earlier to
...
\begin{theorem}[Yu and Vahdat]
  ...
  write propagation, there is an ECG history such observed numerical
