...
unreliable for its intended application. For such applications, rather
than viewing consistency as an all-or-nothing proposition, it is
desirable to work with a model that formally quantifies levels of
(in)consistency and provides controls to ``tune'' the application by
tightening or relaxing the allowed amount of divergence between
replicas. The conit framework makes the consistency/availability
tradeoff into a first class concept, allowing applications to choose
where they fall along the conceptual spectrum between total
consistency (i.e. linearizability) and total availability (i.e. never
blocking to enforce consistency requirements). Mechanisms to exchange
either consistency or availability for more of the other, perhaps in
response to factors like application workload and network capacity,
provide advantages for distributed shared memory frameworks deployed
across a disruption-prone network, where a level of global
...
write access submitted to it by clients of the application; in this
case, $P$ is said to be the \emph{originating replica} of the access,
and all other processes are \emph{remote} for that access. A read is
always handled locally by the originating process, without notifying
any remotes. However, since a write affects the shared data, the
originating process of a write must eventually notify all remote
replicas about the access. Likewise, it must be notified of writes
originating at other processes. Furthermore, because the order of
writes matters for many underlying data models, in general the writes
must be delivered everywhere in a total order to keep the state
machine replicas in harmony. Thus, a mechanism for reliable eventual
totally-ordered delivery is required to propagate write accesses
between the processes in $\AllProc$---recall from Section
...
\ref{ssec:tsae-message-ordering}. First, there is a notion of
``compulsory'' anti-entropy sessions, which are required because pure
TSAE only requires that sessions run periodically, so it cannot make
guarantees about when this happens. Second, compulsory anti-entropy
sessions can be one-way, either pull- or push-based, with one side
...
\ref{sssec:write-propagation}, provides the same progress guarantee as
TSAE+TO: assuming no permanent host or network failures, all processes
will eventually apply each write to their local replica in the same
order. By itself, this implements a \emph{weak} (or \emph{eventual})
consistency model, where replicas are allowed to diverge temporarily,
but will eventually converge to global agreement, at least if they can
communicate and new updates do not outpace the rate of
convergence. However, weak consistency does not say anything about how
far apart replicas might diverge in the meantime. Thus, the majority
of this section is concerned with supplemental mechanisms used to
bound the divergence between replicas at any moment. These mechanisms
function by triggering so-called \emph{compulsory} anti-entropy
sessions. As in ordinary TSAE, an application may choose to implement
so-called \emph{voluntary} anti-entropy sessions at any time, which
may improve performance. However, only compulsory sessions are
required for the correctness of the protocol. The correctness
guarantees enforced by the protocol will be described more precisely
throughout Sections
\ref{ssec:conit-numerical-consistency}--\ref{ssec:conit-correctness}.
...
Intuitively, the goal of the conit framework is to bound how far apart
each replica is from its ``ideal'' value at any moment. The ideal
value of a replicated state machine is the state it would be in if it
has received all updates---in this context, write accesses that
correspond to database state transitions---and applied them locally in
the correct order. In the conit framework, there are two sources of
error that would cause a replica to diverge from its ideal value:
\begin{itemize}
...
\item \textbf{Out of order updates}: The error attributable to
  ...
  first update applied in an order different from the final order
\end{itemize}
...
right away, making $Q$'s replica stale. Out of order updates are
possible because when $P$ receives an update, whether submitted by a
client locally or forwarded to $P$ by another process during
anti-entropy, $P$ generally applies the update to its own replica
immediately, \emph{bypassing} the total-order mechanism that
determines the final global order of updates. This generally increases
performance because processes do not have to wait for the TO component
of TSAE+TO to determine the final order of writes.

Naturally, the possibility of incorrectly ordered updates necessitates
a mechanism to ``roll back'' these updates so they can be reapplied in
the correct order, possibly leading to different results. A rollback
happens if the TO mechanism, after deciding the final, globally
consistent order of updates seen so far by $P$, determines that some
of them have been applied locally in the wrong order. Updates that
have been applied before confirming their final order are said to be
\emph{tentative}, since they are subject to being rolled back and
applied in a different order. At some point, tentative updates become
\emph{committed}, possibly after performing a rollback-and-reapply
process to correct the state of the underlying state
machine. Committed updates are never rolled back. The commitment
process and its correctness properties will be elaborated on in
Section \ref{ssec:conit-order-consistency}.
...
\subsubsection{Local and Ideal Histories}
...
write log, introduced below. The most general form of access is a
``write'' access that reads values in the database, modifies some of
them, and returns a value to the user.
...
$D$ is denoted $D + w$. Because reads are just special writes that
happen not to modify the underlying database, we have that $D + r = D$
if $r$ is a read.
...
history order. Thus, the database state at a process $P$ at any
moment, written $\Dstate{P}$, is defined
\begin{equation}
...
\eqref{eq:database-state-P} always holds.

We will define an \emph{ideal history} as
\begin{gather}
  \Dideal = \Dinit + \Hglobal \label{eq:conit-D-ideal}
\end{gather}

The global history, as a set, is the collection of all local
histories in the system:
\begin{equation}
  \Hglobal = \langle \bigcup_{X \in \AllProc} \localhistory{X}, < \rangle
\end{equation}

The correctness properties of the conit framework take the form,
``There exists a global history such that each access observes a
level of consistency value within $\epsilon$ of the value it would
have returned in the global history.''
...
between the $\localhistory{P}$ and $\WL{P}$ is that the former is an
...
the local replica and $w$ is added to $\WL{P}$. Since $D + r = D$ if
$r$ represents a read access, Equation \eqref{eq:database-state-P}
implies the following equation defining $\Dstate{P}$ in terms of the write
log:
\begin{equation}
  \Dstate{P} = \Dinit + \WL{P} \label{eq:conit-DP}
\end{equation}

The mechanism used to propagate writes among replicas is TSAE. Thus,
each processes maintains the datastructures required of TSAE, whether
that be the summary and acknowledgement vectors of Sections
\ref{ssec:tsae-summary-vectors} and \ref{ssec:tsae-acknowledgement},
or the matrices of Section \ref{ssec:tsae-unsynchronized}. The write
log plays the role of the message log in the TSAE protocol, with
writes stamped with the identifier and the (physical or logical) clock
value of the originating processes at the time it is
submitted. Following TSAE, $P$ uses the summary vectors of remote
replicas to determine which writes it may need to send them, and
likewise sends it summary vector to others to receive updates from
them. Messages are only ever sent it batches determined by comparing
summary vectors, and properties like the coverage property
(c.f. \ref{ssec:tsae-summary-vectors}) are maintained as invariants.

In Section \ref{ssec:tsae-message-ordering}, we made a distinction
between a message that is merely received into the log and one that
has been delivered to the application in a predictable order. In the
protocol we describe, write updates are applied to the replica upon
receipt into the log (note that this is reflected in Equation
\eqref{eq:conit-DP}), before their position in the final total order
is necessarily known. The total-order component of TSAE+TO is
periodically run to determine a global final order of write updates,
but instead of deciding when messages can be \emph{delivered} to the
application, we say the TO component decides when writes can be
\emph{committed}, meaning there is no longer any ambiguity about their
position in the global total order. When messages are ready to be
...
We assume there is an ideal global history $\Hglobal$.
...
Note that $\localhistory{P}$ may later be rewritten during by write
commitment component of the protocol, but the observed prefix history
is static and reflects the state of $\Dstate{P}$ when $a$ was originally
applied at $P$ and a return value may have been given to the user.

\begin{definition}
  The \emph{ideal prefix history} $\PHideal{\left(a\right)}$ of an
  access $a$ is defined as the prefix of $a$ as it occurs in some
  (assumed given) ideal history $\Hglobal$. Similarly, the \emph{ideal
    state} of an access is defined as the state of the database at the
  time $a$ is applied in the ideal history:
\begin{gather}
...
application as long as $\Dstate{P}$ deviates from its ideal state by at most
some margin of error $\epsilon \geq 0$. If the divergence is any
greater, the data at $P$ is unreliable and the application may become
unavailable until consistency is re-established. Alternatively, the
application may remain available but alert the user to the
unreliability of the data, which may affect how the user interprets
and interacts with the application.

The goal of the conit model is to provide a method of quantifying
inconsistency. However, applications have unique data models---note
that we have not made any assumptions about the format of the backing
data store, such as whether it is a traditional relational database, a
key-value store (such as a simple memory model that associates names
like $x, y, z\ldots$ with numerical values), or even a bespoke data
model uniquely designed for the application. This poses a challenge
when designing a general framework to measure divergence without
making detailed assumptions about the nature of the data.
...
some parts of the data, but disagree on other parts, and so it is
desirable to consider a more granular model of consistency than one
that measures the consistency at the level of the whole replica. The
conit framework strikes a balance between being general enough to work
in applications with different data models and practical enough to
efficiently enforce consistency with simple protocols.
...
\begin{description}
\item[Push based] $P$ requests the summary vector of $Q$ and uses
  this information to decide which writes $P$ is aware of that $Q$
  has not been informed about, pushing these updates to $Q$
\item[Pull based] $P$ sends it summary vector to a remote replica
  $Q$, which $Q$ uses to compute a set of writes to send to $P$
\end{description}
...
through innovations in hardware such as mesh radios, combined with
low-level protocols designed for mobile ad-hoc and disruption-heavy
networks. However, these non-traditional network architectures and
extreme operational environments raise higher-level questions about
how to design distributed applications that communicate over these
...
are not a good fit for this sort of environment. This implication
...
the geographical distance between the agents. Strong consistency
models are well-suited to scenarios where a centralized database can
be accessed through a small number of dedicated servers, but locality
suggests that agents at the tactical edge will face difficulty
maintaining communication with central servers. Alternatively, for
resilience and efficiency, multiple replicas of a shared database can
be maintained and deployed at strategic locations in the
network. However, the high coordination overhead required to keep
these replicas consistent with respect to strong memory models is
prohibitive. We concluded that weaker consistency models are called
for in these settings.

Brewer's CAP theorem, a classic cornerstone result in distributed
systems theory, seemingly presents a major challenge when strong
consistency is unattainable. A simplistic interpretation of the
theorem suggests that when strong consistency is sacrificed in favor
of system performance, system designers are left with little control
over the relative consistency between different replicas of common
data. This is an uncomfortable conclusion, because the appreciable
safety concerns in disaster response environments demand control over
the consistency of shared information. That is, a common operating
picture is a prerequisite to effective coordination between
agents. The CAP theorem seems to suggest the application designer is
left stuck between a rock and a hard place.
...
delays. This content provides a foundation building resilient,
