...
\ref{sec:message-passing}, which we think of as analogous to Internet
...
\ref{ssec:tsae-message-ordering}. The resulting system provides
...
presented in Section \ref{sec:consistency-units} is that the middleware provides the
...
\ref{sec:consistency-units}, conventional scalar logical clocks can
...
\ref{ssec:conit-real-time-consistency} requires each process to keep
...
\ref{sec:consistency-units} prescribes a policy for TSAE based on
...
Section \ref{ssec:vector-clocks}) with a few differences. First, the
...
Section \ref{sec:consistency-units}, this value is called $P$'s \emph{commit
...
\begin{proof}
  ...
  \ref{ssec:tsae-message-log} ensures each message is eventually
...
model (\ref{ssec:conit-system-model}), its three dimensions of
consistency
...
We mentioned in Section \ref{ssec:allocation-of-network-resource} our
