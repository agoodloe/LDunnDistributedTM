...
$\WL{Q}$ that $P$ has not seen, leaving the two sides in
...
$Q$. Slightly different from Golding's presentation, the figures
discussed in Example \ref{ex:tsae} also demonstrate a slight
...
$\min_{X \in \AllProc} \left(\summary{P}{X}\right)$. $Q$ does the
same. Furthermore, $\ack{P}{Q}$ and $\ack{Q}{P}$ are set to this
value. This is correct because it preserves the invariant that they
are respectively lower bounds of $\ack{Q}{Q}$ and $\ack{P}{P}$.\footnote{There is a
  subtle potential race condition here: $\ack{P}{Q}$ and $\ack{Q}{P}$
  have to be computed from the minimum of the pointwise maximum of the
  summary vectors exchanged at \emph{beginning} of the anti-entropy
  session, and not their current values after. Otherwise, the values
  could incorrectly reflect information $P$ and $Q$ learned from third
  parties during their anti-entropy session.}
The following two lemmas explain the utility of $\ackVec{P}$ as a
