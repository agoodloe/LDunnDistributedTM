...
symmetrically. For purposes of continuous consistency, below
., this
optimization implements the ``view advance'' mechanism used to bound
numerical error.
...
consistency is re-established by running \emph{compulsory}
anti-entropy. However, applications have unique data models and
...
describe the framework now in terms of its system model
...
of as equal to $D$ if $r$ is a read) . Thus, the database state at a
process $P$ at any moment, written $D_P$, is always $\Dinit + H_P$.
...
\begin{figure}
  ...
  \caption{Figure for Example \ref{ex:conit-numerical} where
    messages with a grid background represent this process' view of $\WL{C}$.}
...
\begin{example}
  ...
  the messages shown with a grid pattern represent this process' view of
