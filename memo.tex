...
\begin{definition}[Vector comparison]
  Let $v, w$ be two vector timestamps. We define the following relations:
  \begin{align*}
...
known to another processes, it is often because a process wants to
compute which facts are known to \emph{all} other processes. Sections
\ref{sec:tsae} and \ref{sec:continuous-consistency} discuss an example
where mechanisms similar to vector and matrix clocks, called version
vectors and matrices, can be used in the context of database
replication, where they drive decision-making about which updates need
to be propagated and when they can be purged from a write log.
...
\begin{lemma}[Causal consistency is CAP-available]
  ...
  is, causal consistency is not subject to the CAP theorem.
\end{lemma}
\begin{proof}
...
\ref{sec:continuous-consistency}, timestamps from conventional scalar
logical clocks can be used everywhere in place of physical
clocks. Indeed, this seems to be how Yu and Vahdat implement TSAE as
...
\ref{ssec:tsae-unsynchronized} explains one way (introduced by
Golding) that the assumption of synchronization can be removed.
...
knowledge of $Q$ to additionally reflect any new messages $P$ has just
pushed during anti-entropy. $Q$ updates $\ackMatrix{Q}[P]$
symmetrically. For purposes of continuous consistency, below, this
subroutine implements the ``view advance'' mechanism used
...
network. We suggest that the conit framework has advantages for
...
most some margin of error $\epsilon \geq 0$. If the divergence is any
greater, then the data is unreliable and the application may become
unavailable (or otherwise alert the user to the potential
unreliability of the data) until consistency is re-established by
running \emph{compulsory} anti-entropy. However, applications have
unique data models and measure this error in different ways, so
building a general framework to enforce quantitative error bounds on
replicas without making specific assumptions about the nature of the
data is challenging.
...
basis, perhaps based on feedback and observations from the network.
...
shared data, which we assume is replicated in full at each site. By
itself, using TSAE+TO to propagate updates provides only a progress
guarantee: all updates will eventually be received, acknowledged,
delivered, and purged from the write log of all processes, assuming
the devices do not fail and network partitions do not last
forever. This implements a \emph{weak consistency} model, where
replicas are allowed to diverge temporarily, but will eventually
converge to an agreement if allowed to communicate, at least if
updates do not outpace the rate of convergence. Unfortunately, weak
consistency does not say anything about how far apart replicas might
diverge in the meantime.

A precise statement of the correctness of the model is somewhat subtle
and we defer the issue until below (\ref{}). Recall from Section
\ref{ssec:tsae-message-ordering} equivalent state machines beginning
in the same initial state will agree assuming all machines have
applied the same transitions in the same order. Thus, there are two
explanations for $D_P \neq \Dideal$, and hence while ideal prefix
histories may different from observed histories: Either there are
writes $w \in \Writes$ that are not contained in $\WL{P}$, or the
writes in $\WL{P}$ have been applied in a different order that
...
\begin{itemize}
\item There must an upper bound on the set of updates that $P$ has not
  seen
\item There must be an upper bound on how many updates $P$ has
  received by applied in the wrong order
\end{itemize}
...
\emph{accesses}. Reads and writes are handled by the protocol
differently. Performing a write access advances the state machine,
while a read examines the current state. Recall that a history is an
...
Processes handle read and write access differently. A read access
submitted to $P$ by a user is always handled locally by $P$ without
involving other processes. The read action is immediately performed
(while enforcing consistency requirements using the protocols below)
and its value is returned to the user. When a read request is
submitted to $P$, the requested value is read immediately from $D_P$
and returned to the caller.  With either type of access, $P$ is said
to be the \emph{originating replica}, and all other processes are
\emph{remote}. The value of $D_P$ is said to be the \emph{observed
  state} of the access.

Writes in general should be thought of database transactions
containing multiple instructions to be executed against the database,
possibly containing embedded application logic. For example, a write
may read a value in the database and make a business decision whether
to increment it.The result of applying write action $w$ to a database
state $D$ is denoted $D + w$. Let $D + H$ denote the result of
applying each of the writes in $H$ to $D$ in history order. Thus, the
database state at a process $P$ at any moment, written $D_P$, is
always $\Dinit + H_P$. Read accesses can be thought of as a special
case of write access that do not modify the underlying database state,
meaning $D + r = D$ if $r$ is a read.
...
\[\Val^F(D + w) = \Val^F(D) + \NumWeight(w, F).\] Combining this with
Equations \eqref{eq:conit-DP} and \eqref{eq:conit-Dideal} gives
\begin{gather}
  V_P = \Vinit + \sum \{\NumWeight(w, F) | w \in \WL{P}\} \\
  \Videal = \Vinit + \sum \{\NumWeight(w, F) | w \in \WLideal\}
\end{gather}

The correctness of numerical error is
\begin{equation}
  | \Videal - V_P | < \NE\left(P, F \right)
\end{equation}

Naturally, enforcing this condition requires bounding the set of
writes that $P$ has not seen:
\[
  \WLideal \setminus \WL{P}
\]

The mechanism used requires each process to maintain approximate
(conservative) knowledge of $\WL{P}$, and then bound its own
``contribution'' towards the writes in $\WLideal$ that are not in
$\WL{P}$.

\subsubsection{Assumptions}
Bounding numerical error requires a cooperation between all processes that process write updates to that conit.
...
implementing views for the $\ackVec{P}$ and $\ackMatrix{P}$
...
\item If TSAE is implemented with $\ackVec{P}{}$, then $P$ must
  ...
  of the last push to $Q$. $P$ conservative estimates $Q$ has not received any
...
\item If TSAE is implemented with matrices, $P$ estimates that $Q$ has
...
$\WL{Q}{P}$ is required. For the $\ackVec{}$-based implementation of
...
\begin{gather}
  ...
  \twn{P}{Q, F} \equiv \sum_{\substack{
    w \in \EstUnseen{\left(P,Q\right)} \\
    \NumWeight\left(w, F\right) < 0}} \NumWeight\left(w, F\right)
%\twn{P}{\left(Q, F\right)} \equiv \sum_{ w \in \WLat{P}{P} \setminus \View{P}{Q}} \{ \NumWeight\left(w, F\right) | \NumWeight\left(w, F\right) < 0\}
\end{gather}
The motivation for totally positive and negative values will be
explained below. To bound numerical error, the intuition is that $P$'s
estimate of unseen numerical weight must not exceed $Q$'s allowable
error, divided by $|\AllProc| - 1$ (the number of processes minus
one). This value represents $P$'s share of allowable weight of writes
accepted by processes other than $Q$ that are unseen by $Q$. When $P$
submits a new write, $P$ first checks for whether the following
conditions hold:
\begin{align*}
  \twp{P}{Q, F} + \NumWeight(w, F) &\leq \frac{\NE\left(Q, F\right)}{|\AllProc| - 1} & \textrm{if $\NumWeight(w, F) > 0$} \\
  \twn{P}{Q, F} + \NumWeight(w, F) &\geq - \frac{\NE\left(Q, F\right)}{|\AllProc| - 1} & \textrm{if $\NumWeight(w, F) < 0$}
\end{align*}
...
Positive and negative weights must be totalled separately rather than
allowing updates with positive weight to ``cancel'' out ones with
negative weight. This is because $P$'s view is conservative. For
example, suppose $P$ has accepted writes with positive and negative
weights that exactly cancel out, and suppose $Q$ has actually seen all
of the weights with positive weight. If $P$ totalled all weights
together, it would wrongly estimate that the weight of writes unseen
by $Q$ is $0$, while the actual weight of unseen writes may exceed
$Q$'s error bounds.
...
algorithm only relies on $P$'s local knowledge. How to do bound
relative error efficiently without knowing the ideal value
$V_{\textrm{final}}$ is non-obvious.
...
\begin{example}
  ...
  $A$'s view of $\WL{C}$ (we assume $A$ maintains a matrix, so it has
  an estimate of $\WLat{C}{X}$ for each $X$). Here, $A$ estimates
  there are two messages in $\WLat{A}{A}$ that are not in
  $\WLat{C}{A}$, with timestamps 6 and 9. While handling access
  $(A, 9)$, before returning to the user, the combined weight of the
  unseen updates to any conit $F$ must not exceed
  \mbox{$\NE\left(C, F \right)/ 2$}, reflecting $A$'s ``share''
  (between $A$ and $B$) of the total allowable error.  If this limit
  is exceeded, $A$ would become unavailable and engage in compulsory
  anti-entropy with $C$.\footnote{If $A$ is multithreaded and can
    handle multiple accesses at once, then $A$ only has to be
    unavailable for further access involving the same underlying
    conits. Reads and writes only affecting other conits are not
    blocked.} Note that $A$'s view of $\WLat{C}{B}$ does not play a
  role, as $A$ only works to bound the weight of its own writes unseen
  by $C$.
\end{example}
...
replica state reflects this order. TSAE+TO guarantees that eventually
writes will be committed, and the order of committed writes in the
same everywhere. Until they are committed, the tentative writes might
have been applied in the wrong order. Writes applied in the wrong
...
\begin{example}
  Suppose an airline reservation application is used to book
  individual seats for a flight. Suppose $P$ and $Q$ are two web
  ...
  of transitions:
  \[ (\Dinit + w_Q) \mapsto \Dinit \mapsto (\Dinit + w_P +
    w_Q) \]

Semantically, the reader should think of the database state
$\Dinit + w_P + w_Q$ as meaning ``Seat \#1 is reserved to $P$'s
client, while $Q$'s client attempts to double book the seat and is
denied.'' The rollback process at $Q$ may be associated with business
rules triggering a notification to $Q$'s client that their reservation
has been cancelled, alongside a refund and compensation fee.

Clearly, accepting $w_Q$ in a tentative state represented a liability
for $Q$. The purpose of controlling order error is precisely to upper
bound the potential cost associated with a rollback operation.
\end{example}

Unlike numerical error, order error can be bound without
cooperation. We only need to assume that processes, when required, can
invoke a write commitment mechanism to learn the final order of their
tentatively accepted updates.
\begin{quote}
  \textbf{Ability to commit writes}: While handling an access, each
  process has a mechanism to determine the final order of any writes
  in its write log.
\end{quote}
For the implementation we have described, $Q$ can commit writes by
engaging in pull-based anti-entropy. While handling a write with
timestamp $t$, then $Q$ can pull writes from other processes until $t$
is the minimum entry in $\summaryVec{P}$ and therefore
$t \leq \ack{P}{P}$, at which point the final order of the write is
known. Note that this works regardless of whether clocks are
implemented as physically synchronized clocks or scalar logical
clocks---in either case, $\summary{P}{X}$ will be greater than $t$
after pull-based anti-entropy with $X$.

Each write is $w$ is associated with an \emph{order weight}, intended
to capture the cost of rolling back $w$ and reapplying it in a
different order than the one it was first applied. Let $r$ be a read
access submitted to $P$ with a dependency on conit set $\mathcal{F}$.
...
in
\[\Uncommitted{P} \subseteq \PH_{\textrm{obs}} \setminus \left(\PH_{\textrm{obs}} \cap
    \PH_{\textrm{ideal}}\right)\] is a subset of the uncommitted
writes. For a read access that depends on conits in $\Conits$, we only
care about the out-of-order writes that affect any of the conits in
$\Conits$. Thus, the actual cost of rolling back writes is given by
the above set, where all histories are restricted to the writes
actually affect $\Conits$:
\begin{equation}
  \PH_{\textrm{obs}}|_\Conits - \left(\PH_{\textrm{obs}}|_\Conits \cap
     \PH_{\textrm{ideal}}|_\Conits \right)
 \end{equation}
...
\begin{definition}[Order error]
  ...
  \[
    \mathsf{OrderError}\left(r, \mathcal{F}\right) \equiv \sum_{F \in \mathcal{F}} \left\{\OrderWeight\left(w, F\right) | w) \right\}
  \]
\end{definition}


Thus, we can bound the uncommitted order weight on a conit $F$ is
defined as the sum
\[
  \EstOrdWeight = \sum_{F \in \mathcal{F}} \{\mathsf{OrdWeight}\left(w, F\right) | w \in \Uncommitted{P}|_{\mathcal{F}}\}
\]

\begin{comment}
et $\PH_{\textrm{obs}}$ be the observed
...
$\mathsf{OrderError}\left(r,\mathcal{F}\right)$. To bound order error, it is
...
\begin{example}
  ...
  The next access processed by $A$ will observe an incorrect prefix
  history of
  \[\PH_{\textrm{obs}} - \left(\PH_{\textrm{obs}} \cap
      \PH_{\textrm{ideal}}\right).\] This consists of all writes after
  and including $(A, 6)$, the point at which the write log deviates
  from the ideal history (seen in Figure
  \ref{fig:conit-order-b}). This corresponds to an actual order error
  of $4$. Note that the message $(A, 3)$ happens to be in its ideal
...
\begin{definition}[Causal precedence of accesses]
  ...
  could have influenced how $A'$ executed.\footnote{At face value it
    ...
    isolated database transactions, it is possible that $A$ logically
    takes effect before $A'$, although the transactions may be
...
differ in their guarantees. The first is a two-phase locking procedure
where, during write propagation, remote locks covering all of the
affected data items are acquired from all hosts where one intends to
send write updates. Once all locks have been acquired, all necessary
writes are sent. While locks are held, reads from affected conits are
blocked on those remotes. When the writes are propagated, locks are
released. This two-phase locking policy ensures that the newly seen
write operation appear to take effect at all replicas at the same
time. The two-round procedure leads to the following characterization
of the conit protocol.
\begin{theorem}[Yu and Vahdat]
  Assuming the two-phase locking policy is used during compulsory
  write propgation, there is an ECG history such that the numerical
  and order error of every access is correctly bounded with respect to
  its ideal value in the ECG history.
\end{theorem}
...
Allowing unbounded numerical error captures one-copy serializability
(1SR), which is loosely a form of sequential consistency for
replicated databases. \citationneeded
\begin{theorem}
