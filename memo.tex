...
across a disruption-prone network. In these contexts, message-passing
...
access a value of $D_P$ whose divergence from its correct or ``ideal''
value is limited by an upper bound. The ideal value of $D_P$ is the
state it would be in if it has received all updates and applied them
locally in the correct order. In the conit framework, there are two
sources of error that can cause a replica to diverge from its ideal
value:
\begin{itemize}
...
separate from the numerical weight, and may be thought of as a cost
associated rolling back an update if it is later found to have been
applied in the wrong order. Out-of-order error and the commitment
process are the subject of Section \ref{ssec:conit-order-consistency}.
...
conit significantly and another only slightly but not affect a third
conit at all. Likewise, each process specifies its tolerable
...
with a separate numerical weight for each conit: Likewise, the real-time staleness and out-of-order
...
% A process that enforces zero order weight will never bypass the total order mechanism.)
...
This generally increases performance because processes do not have to
wait for the TO component of TSAE+TO to determine the final order of
writes. The possibility of incorrectly ordered updates necessitates a
mechanism to ``roll back'' these updates so they can be reapplied in
...
of them have been applied locally in the wrong order.
...
We imagine these processes communicate over a
...
\subsubsection{Enforcing Bounds on Divergence}
...
\subsubsection{Tuning Bounds on Divergence}
\label{sssec:conit-tuning-bounds}
