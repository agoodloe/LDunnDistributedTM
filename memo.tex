...
divergence exceeds a certain treshold, the data becomes too unreliable
for its intended application. For such applications, rather than
viewing consistency as an all-or-nothing proposition, it is desirable
to work with a model that formally quantifies levels of
(in)consistency and provides controls to ``tune'' the model by
tightening or relaxing the allowed amount of divergence. The conit
framework makes the consistency/availability tradeoff into a first
class concept, allowing applications to choose where they fall along
the spectrum between total consistency (i.e. linearizability) and
total availability (i.e. never blocking to enforce consistency
requirements). Mechanisms to trade consistency for availability, or
vice versa, perhaps in response to factors like application workload
...
request a data \emph{access}. Any process may handle a read or write
access submitted by clients of the application. A read access is
always handled locally, without notifying other processes, but since a
write affects the shared data, every process must eventually be
notified of every write. Furthermore, because the order of writes
matters for many underlying data models, in general the writes must be
delivered everywhere in a total order. Thus, a mechanism for reliable
eventual totally-ordered delivery is required to propagate writes
between processes---recall from Section
\ref{ssec:tsae-message-ordering} that this provides a general method
of replicating a state machine.  The implementation we describe here
is the one proposed by Yu and Vahdat, which is based on TSAE+TO,
timestamped anti-entropy with a totally ordered delivery component,
though other solutions can be used instead.

The write propagation mechanism, described in detail in
\ref{sssec:write-propagation}, provides a progress guarantee: assuming
no permanent host or network failures, all processes will eventually
apply each write to their local replica in the same order. By itself,
this implements a \emph{weak consistency} (or \emph{eventual
  consistency}) model, where replicas are allowed to diverge
temporarily, but will eventually converge to global agreement, at
least if they can communicate and updates do not outpace the rate of
...
bound the divergence between replicas at any moment. In the
implementation we describe, these mechanisms function by triggering
so-called \emph{compulsory} anti-entropy sessions, as opposed to
voluntary anti-entropy sessions which may happen at any time but do
not provide guarantees about when and how often they propagate
updates. Voluntary anti-entropy sessions are not required for
correctness of the implementation, but the application may use them in
the background to improve performance.
...
The ``ideal value'' of a replicated state machine is the state it
would be in if it has received all updates (state transitions) and
applied them in the correct order. In the conit framework, there are
two sources of error that would cause a replica to diverge from its
ideal value:
\begin{itemize}
\item \textbf{Unseen updates}: The error attributable to state
  transitions that have not been applied to the replica because the
  host process has not been notified of them yet
\item \textbf{Out of order updates}: The error attributable to
  transitions the machine has applied in the wrong order, leading it
  down an ``errant path'' beginning at the time of the first update
  applied in an order different from the final order
\end{itemize}
Unseen updates occur when a write access is submitted by a client at
one process $P$, called the originating process, but another process
$Q$ is not notified right away, making $Q$'s replica somewhat
stale. Out of order updates are possible because when $P$ receives an
update (whether submitted by client at $P$, or forwarded to $P$ by
another process $Q$), it generally applies the update to its own
replica immediately, \emph{bypassing} the total-order mechanism that
...
to determine the final order of writes. However, this usage
necessitates a mechanism to ``roll back'' updates that were applied in
the wrong order. A rollback happens if the TO mechanism, after
deciding the final global order the updates seen by $P$, determines
that some updates have been applied locally in the wrong
order. Updates that have been applied before confirming their final
order are said to be \emph{tentative}. At some point, tentative
updates become \emph{committed}, possibly after rolling back
out-of-order tentative updates and re-applying them in the correct
order. Committed updates are never rolled back. The commitment process
will be elaborated on in Section
\ref{ssec:conit-order-consistency}.\footnote{To a first approximation,
  the implementation described in Section
  \ref{ssec:conit-order-consistency} guarantees committed writes are
  applied in a global total order. However, it does not make
  guarantees about the relative order of \emph{order-independent}
  updates, which we define as those that do not apply non-zero
  \emph{write weight} to any shared conit. It is up to the application
  programmer to ensure that the relative order of order-independent
  updates does not matter for the data model.}
...
\subsubsection{Handling Accesses}
...
and may additionally return a value like read accesses do. In fact, we
define a read access as simply a transaction that does not modify any
data items, but only returns a value. For this reason, read accesses
do not participate in write propagation and are not entered into the
write log (see Section \ref{sssec:write-propagation}); conceptually
they are just a special kind of write access.

A write access can encode application-level logic. For example, it may
read a value in the database and make a business decision whether to
increment it. The resulting state after applying write action $w$ to a
database in state $D$ is denoted $D + w$. Read accesses can be thought
of as special write accesses that do not modify the underlying
database state, meaning $D + r = D$ if $r$ is a read. Recall that a
\emph{history} is an ordered sequence of read and write accesses. The
\emph{local history} of a process $P$, denoted $\localhistory{P}$, is
the sequence of accesses $P$ has applied to $\Dinit$ since the start
of the application. Let $D + H$ denote the result of applying each of
the accesses in $H$ to $D$ in history order. Thus, the database state
at a process $P$ at any moment, written $D_P$, is defined
\begin{equation}
...
\end{equation}
It is assumed that the TO mechanism provides a way of ordering all
write updates.

With either type of access, $P$ is said to be the \emph{originating
  replica} of  the access, and all other processes are \emph{remote}
replicas. The value of $D_P$ is said to be the \emph{observed state}
of the access.

\subsubsection{Measuring Divergence}

Intuitively, clients of process $P$ can usefully operate with the
application as long as $D_P$ deviates from its ideal state by at most
...
and interacts with the application. In either case, the consistency
model must provide a method of quantifying inconsistency, and the
application must have a method of estimating the inconsistency of its
replica by calculating an upper bound on this value.

Unfortunately, applications have unique data models and measure error
in different ways. This poses a challenge when designing a general
framework to enforce error bounds without making detailed assumptions
about the nature of the data. The conit framework strikes a balance
between being general enough to work in applications with different
data models and practical enough to efficiently enforce consistency
with simple protocols.

The logical unit of data whose consistency is measured by the conit
framework is the conit, or consistency unit. Conits are an abstract
concept that get their meaning in terms of how they are used by the
programmer. In terms of the implentation of the model, a conit is
implemented as simply an identifier. If the backing database is, say,
a simple key-value store, then each key can be associated with a
conit. Alternatively, a whole group of keys could form a conit. If the
database is a relational database, then a row or group of rows could
form a conit. Even logical values computed from the data model, such
as the sum of several data items, can be associated with a conit. The
divergence of a replica is always measured in terms of the divergence
of individual conits from their ideal values.

As will be described in \ref{sssec:handling-accesses}, a write access
need not be a simple update to a single data item, but might be a
complex sequence of updates to multiple items, which might involve
high-level application logic that makes decisions based on the current
value of data. Thus the behavior of a write, not just a read, can be
influenced by values in the database. Each (read or write) access
submitted to the replication middleware provides a set of identifiers
describing the conits whose values might influence how the access
behaves.

Seeing as there are two fundamental sources of error, the conit
framework provides mechanisms to control the number of unseen updates
and the number of out-of-order updates applied locally. Each write
access, when submitted to the shared memory middleware layer, comes
with two numerical values: its \emph{numerical weight} and its
\emph{order weight}. Actually,
...
push-based and requires cooperation while the latter is pull-based and
handled locally by each process. Controlling real-time staleness by
...
\begin{enumerate}
\item The observed order error and observed real-time error of the
  access are bounded using compulsory pull-based anti-entropy
  sessions to pull updates from remote replicas, if necessary.
\item The access is applied to the local replica, which in general will yield some return value $x$.
\item If the access is a write access, it is pushed onto the write
  log. If necessary, the write will be immediately pushed to remote
  replicas using compulsory push-based anti-entropy to bound their
  observed numerical error.
\item The request returns back to the user, including the its final
  return value.
\end{enumerate}


\subsubsection{Write Propagation}
\label{sssec:write-propagation}
...
they were applied. The reader should notice then that $\WL{P}$ is
precisely $\localhistory{P}$ restricted to write accesses. The
distinction between the $\localhistory{P}$ and $\WL{P}$ is that the
former is an abstract concept and contains accesses of both types,
whereas $\WL{P}$ is an actual data structure maintained by the
algorithms and only contains writes, since (by definition) only writes
affect the database state and therefore need to participate in write
propagation.
\begin{equation}
...
processes. In particular, processes need to know every other process's
...
\begin{equation*}
  \EstUnseen^{P}{\left(Q\right)} \equiv \WLat{P}{P} \setminus \View{P}{Q}
\end{equation*}
...
\begin{gather}
  \EstUnseen^P|^+_F{\left(Q\right)} \{ w \in \EstUnseen^{P}{\left(Q\right)} | \NumWeight\left(w, F\right) > 0 \} \\
  \EstUnseen^P|^-_F{\left(Q\right)} \{ w \in \EstUnseen^{P}{\left(Q\right)} | \NumWeight\left(w, F\right) < 0 \}
\end{gather}
...
not exceed an alloted ``share'' of $Q$'s total error bound. Since
there are $|\AllProc| - 1$ processes that can accept writes without
immediately adding them to $\WL{Q}$, this means the total weight of
unseen messages accepted by each process must not exceed
$\frac{\NE\left(Q, F \right)}{|\AllProc| - 1}$.  When $P$ submits a
...
will be zero.\footnote{The exact handling of write propagation,
  ...
  involves orthogonal implementation details, such as whether to
  acquire remote locks on data items, that will be discussed
  more in Section \ref{ssec:conit-correctness}.}
...
component at $R$ discovers that $w_P$ should be first. Numerical
error only controls the updates a process sees, but not whether they
