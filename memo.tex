...
% Include this to suppress warning
\captionsetup{
  font=normalsize,
  labelfont=bf,
  labelsep=colon,
...
\input{macros.tex}
\usepackage[backend=biber,style=numeric]{biblatex}  % or style=abbreviated equivalent
...
\AtEveryBibitem{%
  ...
  {}
  {\clearfield{url}}}
\addbibresource{bibliography.bib}  % no file extension needed in newer versions, but this is safest
...
\AuthorAffiliation{Lawrence Dunn \\ Department of Computer and Information
  Science \\  University of Pennsylvania,  Philadelphia, PA \\ [10pt]
   Alwyn Goodloe\\ NASA Langley Research Center, Hampton, Virginia
}
...
\Number{0005643}              % Report number
\Month{07}                   % two digit number
\Year{2025}                  % four digit number
\SubjectTerms{Distributed Systems, Formal Methods, Logic, }     % 4-5 comma separated words
\Pages{101}                   % all the pages from the front to back covers
...
\SupplementaryNotes{}
\Acknowledgment{The first author conducted the work during summer internships in 2022 and 2023 at the NASA Langley Research Center in the Safety-Critical Avionics Systems Branch focusing on distributed computing issues arising in the Safety Demonstrator challenge in the NASA Aeronautics System Wide Safety (SWS) program. The authors would like to thank Kari Greer, Andreas Johansson, and Ron Kemnow, who provided the photographs in Figures 1, 2, and 3, respectively.}
...
\abstract{The System Wide Safety (SWS) program has been investigating
  ...
  the essential concepts behind the continuous consistency model for shared memory proposed by Yu and Vahdat,
...
\begin{figure}
  ...
  \caption{A DC-10 airtanker, rated for 9,400 gallons, drops retardant
    above Greer, Arizona. Image credit: Kari Greer/US Forest Service.}\label{fig:airtanker}
\end{figure}
...
\begin{figure}
  ...
  \caption{Screenshot of a firefighter using TAK, where the left
    ...
    vehicle. Image credit: Andreas ``AJ'' Johansson}\label{fig:atak}
\end{figure}
% TODO: How to cite picture?
...
\emph{continuity}. A continuous system can flexibly adapt to its
...
\begin{figure}[t]
  \centering
  \includegraphics[scale=0.5]{images/ironside-kemnow.jpg}
  \caption{The Ironside Mountain lookout and radio repeater station,
    shown here in September 2009, was destroyed by wildfire in 2021. Image credit: Ron Kemnow}
  \label{fig:ironside}
\end{figure}
% TODO: How to cite picture?
...
As aircraft generally have better line-of-sight to ground crews than
...
radio repeaters. In the future, this sort of service could be
...
consumers of data, and this data will need to be  processed by humans and
...
% pdfTeX warning (ext4): destination with the same identifier (name{equation.3.1}) has been already used, duplicate ignored
% seems to arise somewhere between the \begin{example} and \end{example} below
\begin{example}
...
about what other processes know? In particular, it is often of interest to 
compute which facts are known to \emph{all} other processes.
Vector clocks can be used for this purpose. In some contexts,
we must take the idea one step further: what does a process know 
about what other processes know \emph{about other processes}?
This is the purpose of matrix clocks. Sections~\ref{sec:tsae} 
and~\ref{sec:continuous-consistency} feature running examples of
mechanisms similar to vector and matrix clocks, called version 
vectors and matrices. These are used in the context of database
replication, where they drive decision-making about which 
updates need to be propagated and which updates have already been
applied everywhere.
...
similar transport can therefore take FIFO for granted. Providing
...
\begin{example}
  ...
  \item Figure~\ref{fig:ordex-non-fifo} violates FIFO because messages
    $m_1$ and $m_2$ are both sent from $P_1$ to $P_2$, but then arrive
    in the wrong order.
...
One way to implement broadcasting is to send distinct network
...
relates non-overlapping events to their physical times, but does not
...
\begin{definition}[Physical concurrency]
  ...
  precedes the other, in other words if there is some moment in time
...
Figure~\ref{fig:dsm-example-2-linearizations} also depicts an
...
oversimplification is that the CAP theorem represents a ``choose 2
...
subset property is required for correctness.\footnote{Section
...
\begin{landscape}
  \begin{figure}[ht]%For some reason this empty figure adds vertical whitespace that makes the next figure positioned similarly to the ones that follow it.
  \end{figure}
  \begin{figure}[ht]
    \centering
...
are exchanged during anti-entropy sessions just as before, with both
...
advantage of the conit model is that the precise tradeoff can be
...
Intuitively, the goal of the conit framework is to ensure that each
...
uniquely determined by the tag containing the name of their
...
fact that this history must respect external and causal order is
...
$\OErr(P, F)$ bounds the maximum weight of
...
to push updates to $P$. The difference between the implementations has to do with 
 whether the protocol ensures that
...
form of locality to allocate network resources for information
...
delays, share some similarities with real-world environments, such as the
