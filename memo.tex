...
environment. This sort of robust design is challenging due to the fact
that distributed systems designers are forced to make delicate
...
be done. Specifically it proves that strong consistency for a
distributed system makes systemwide network performance an upper bound
on the availability of a system to do useful work for clients. For our
...
will always act with less than perfect global information, a key
...
high packet loss, high error rates, and unpredictable latencies in
message-passing over the communication network(s). A conservative view
suggests expecting the worst performance at particularly inopportune
times. This is because conditions that prompt urgent communication can
be expected to correlate with those that make communication
difficult. For example, tautologically, a communications network is
the most \mbox{congested}, and therefore the least available,
precisely when everyone needs to use it. This phenomenon was most
famously exhibited in the aftermath of the September $11^\textrm{th}$
terrorist attacks, where the sudden demand on the networks virtually
brought them all to a halt (CITE).

From a systems perspective, an unreliable network presents a challenge
for protocols used to coordinate distributed agents. At a high level,
...
from the {\mbox{NIICD}} radio cache \cite{radiocache} and for
whom. Stronger consistency requirements are of course more difficult
to maintain than relaxed requirements because they require exchanging
more information with other agents quickly, putting a heavier burden
on the network. When the network is slow, nodes that need to
communicate may have to wait for network to deliver their messages,
delaying the handling of user requests in the meantime, diminishing
...
the network is unreliable. The fundamental cause is that when faced
with a poor communications environment, agents must choose between
long delays in sending or receiving information, or acting with only
limited knowledge, or typically some amount of both. Both options are
problematic and present safety challenges because operational safety,
by nature, requires agents to gather information about their
environment and react to it. This information is received from distant
sensors and other agents, who send it over the network(s), so poor
network performance is a safety problem.
...
\cite{2019:stickney}, posing a serious danger to firefighters. A 2018
accident led to the death of one firefighter and the injury of three
others when an 87-foot Douglas Fir tree was knocked down by an
...
cannot be guaranteed simultaneously, as in our example: if
firefighters are unable to broadcast their locations to the pilot,
...
two other aspects that are highly pertinent to reasoning about
distributed systems. We pause to draw attention to them.

\paragraph{Epistemology}
Observe that the issue in the VLAT example does not simply disappear
if no ground personnel are actually within 100 meters of a drop
...
\(\textbf{P}_\textrm{safe}\)---\emph{knowledge} of the fact, and not
merely the fact of it, is the crucial part. In philosophical terms,
the logic of distributed agents is inherently an \emph{epistemic} one
(CITE), meaning it must take into account not just what is true but
what is known. The need to share knowledge is what drives
communication and puts a burden on the network.
...
concerned with distributed computing systems used by firefighters and
other disaster response agents. The components of the system could be
computers, smartphones, or other communication devices. These may be
personal devices carried by individuals, mounted to ground or
air-based vehicles, or be deployed as temporary or permanent
infrastructure. The clients of these devices could be civilians,
firefighters, or other agents involved in disaster response
efforts. The components of the system coordinate to solve such tasks
as navigating safely in close proximity, delivering resources to
...
probably geographically dispersed computing and communication devices,
which we shall refer to simply as ``nodes.'' A notable aspect of what
makes the system \emph{distributed} is that the nodes must communicate
by message-passing, which must be facilitated by some kind of
network. This restriction is implied by absence of a common memory
(whereas processes running on the same machine have the option to
share data by writing it to a shared memory location).

A foundational assumption is that the network is unreliable---Section
\ref{sec:disaster-response} explained why this is certainly true in
disaster scenarios.  Message delivery is not instantaneous, so
messages are passed with some unpredictable latency. The latency need
not be finite, as the network may deliver a packet zero times, i.e.~it
may silently delete the packet without alerting the sender. A message
may be also be delivered multiple times, and different messages may
arrive in any order. In the case of \emph{Byzantine faults} (CITE) the
network may even act like a malicious adversary, though we will not
consider this scenario in this document. The main challenge presented
to the system nodes is to coordinate their actions despite this
turbulent environment.

\begin{figure}[h]
  \center \includegraphics[width=\textwidth]{images/messages.png}
  \caption{Time diagram for messages}
  \label{fig:messages}
\end{figure}


To focus on higher-level considerations, we shall abstract away from
the chaotic details of the network and assume that messages are
delivered exactly once, but with an unpredictable delay that cannot be
directly controlled.\footnote{Network transport protocols such as TCP
(CITE) and LTP (CITE) can provide reliable message delivery, so one
could say we are working at a level abstraction above such low-level
protocols.} Figure \ref{fig:messages} shows a time diagram for a
series of four messages sent between three processes. The $x$-axis of
the diagram shows the flow of physical time. Each process consists of
a worldline. Each message $m_n$ starts off as a message \emph{send}
event $m_n.s$ indicating the moment in time the message is sent over
the network by its originating process. The four arrows depict the
temporal delay occurring before the message is received.

\subsubsection{Causality and logical clocks}

\begin{itemize}
  \item Difficulty of synchronization
  \item The ``happens before'' relation
  \item Lamport and vector clocks
\end{itemize}
