...
\textbf{NEED TO CREATE A FIGURE HERE}. Figure X satisfies CO, and
therefore FIFO, but not TO. Figure Y satisfies TO but not CO . Note
that, of course, we can also consider hybrid message order models that
provide both total and causal order.
...
Strong consistency, as defined in Section
...
are tenable, but we have seen that by themselves they provide no upper
bound on the divergence between replicas of data items. Because of the
safety-related nature of our system, we would like to do better than
this.

This section summarizes the key ideas behind continuous consistency,
specifically the conit (``consistency unit'') model proposed by Yu and
Vahdat \citationneeded. Continuous consistency is motivated by the
observation that for many real-world applications it evidently makes
sense to work with data that is consistent up to some
...
quantiative upper bound on inconsistency.

A precise quantitative definition of consistency seems specific to
each possible application. To implement a continuous consistency model
that is not tied to one application, we must first explain how
inconsistency can be quantified generically in the first
place. Second, we must explain how a generic protocols can be used to
enforce. The conit model.

\subsection{The conit model}
\label{ssec:conits}

\begin{itemize}
\item Committed versus uncommitted writes
\item Each process has a vector clock (Section \ref{sssec:vector-clocks})
\item Anti-entropy
\end{itemize}
...
\label{sec:real-time-consistency}
