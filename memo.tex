...
$\PH{\left(a\right)}$, is the set of accesses in $H$ that precede
...
complex application-level logic, the state of $D_P$ observed by $a$
...
they are applied to $D_P$. Remote write accesses added to $\WL{P}$
...
on. These are the conits that logically represent data in $D_P$ that
...
to known which writes $P$ has not seen. Specifically, $Q$ keeps track
of which writes \emph{originating at $Q$} $P$ has not seen. Recall
that $\WLat{P}{Q}$ contains the writes $P$ has received that
originated at $Q$. Therefore, the set of writes originating at $Q$
unseen by $P$, which we call $P$'s actually unseen writes, is defined
by the following set difference:
\begin{equation}
  \ActualUnseen^{Q}{\left(P\right)} \equiv \WLat{Q}{Q} \setminus \WLat{Q}{P}
\end{equation}
Exactly computing the set of unseen writes would require $Q$ to have
perfect knowledge of $\WLat{Q}{P}$, which would be extremely
inefficient. Instead, our second assumption required to bound
numerical error is that $Q$ maintains a conservative estimate, called
its \emph{view}, of this value. $Q$'s view of $P$ is denoted
$\View{Q}{P}$.
\begin{quote}
...
intermediary process $R$, and $Q$ may not know that $P$ has been
informed about these writes. This lower bound corresponds to an upper
bound estimate of the messages it has originated that $Q$ has
\emph{not} received yet. This estimate is defined by the following
equation:
\begin{equation}
...
actually unseen writes in $\WLat{Q}{Q}$. Before the write completes,
$\WL{P}$ will reflect all updates originating at $Q$ before and
including the one being handled, and $\twp{Q}{F}{P}$ and
$\twn{Q}{F}{P}$ will be zero.
