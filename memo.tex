...
We model a distributed system abstractly as a set
\(\mathcal{P} = \{P_i\}_{i\in I}\) of processes which undergo atomic
(indivisible) state changes known as \emph{events}. Events are divided
into three types: internal events, representing state changes inside a
single process, and send and receive events corresponding to messages
passed between processes. It may be helpful to imagine processes as
radios and messages as audio broadcasts, though the framework we are
using is abstract and applies to any kind of communication
technology. To draw out the core issues surrounding messaging, the
diagrams in this section do not depict any internal events.
...
Scalar clocks satisfy the clock condition
...
%\end{lemma}
However, they do not satisfy \eqref{eq:sc}.  While $e$ having a lesser
timestamp than $e'$ rules out $e' \to e$, it does not imply
$e \to e'$. For instance, in Figure
\ref{fig:message-latencies-scalar-b}, $\msend{1}$ has a globally
minimal timestamp value of $1$, but it does not causally precede all
events with timestamps greater than $1$, or indeed any event except
$\mrecv{1}$.
...
%to present the illusion that messages arrive in the right
%order. Thus, we make a distinction between message \emph{receipt} and
%\emph{delivery}. A message is received at a time determined by the
%network, but it is not delivered until the process (which could be
%something like a network protocol driver, a distributed middleware, or
%a user-facing application) is able to enforce whatever ordering
%guarantees it is meant to provide.
...
Consider two features shown in Example
\ref{exmpl:concurrentupdates}. First, there are three operations that
write to the value $x$. For a period of time, $P_1$ and $P_2$ are both
writing to location $x$ at the same time. Second, these operations are
immediately followed by several read requests, whose return values are
not shown in the diagram. A memory model answers the question, ``Which
values might be returned by each of these read requests.'' Such a
model would have to answer several questions, like the following ones:
\begin{enumerate}
\item Do the read operations on $P_1$ and $P_2$ have to the same value?
...
to arrange all systemwide into a total order. A roughly similar
...
This section considers the two major memory models usually considered
...
(returns values consistent with) some sequential arrangement of system
events, maintaining a shared understanding of which operations occur
...
except the latter terminology is specific to transactional databases
and implies other database-related guarantees.
...
\begin{definition}[Linearizable system]
  ...
  system are linearizable according to Definition \ref{def:linearizable}.
\end{definition}

Consider Figure \ref{fig:smEx2} again. If we assume this system
linearizable, then the requirement of global agreement requires $P_1$
and $P_2$ to agree on the order of write events. The requirement of
consistency with external order means that $\memwrite{x}{4}$ must be
ordered before $\memwrite{x}{3}$ and $\memwrite{x}{5}$, but the latter
operations can occur in any order, as long as $P_1$ and $P_2$
agree. They must return responses that are consistent with this common
operating picture. This constrains the system to one of two
possibilities: all the read operations return $3$, or they all return
$5$. Note that linearizability leaves room for non-determinism in the
order of $\memwrite{x}{3}$ and $\memwrite{x}{5}$.
...
The two possible \emph{linearizations} of Figure \ref{fig:smEx1} are
shown in Figure \ref{fig:smEx3}. Possible choices for linearization
points are shown in yellow. Note that only the relative total order of
...
%For many applications this is an unacceptably high penalty. A weaker
% model that is still strong enough for most purposes is
% \emph{sequential} consistency. This is an appropriate model if a
% form of strong consistency is required, but the system is agnostic
% about the precise physical time at which events start and finish,
% provided they occur in a globally agreed upon order.

Next we define \emph{program order}. This is a partial order among
events that orders operations in the order they occur \emph{if they
  occur in the same process}. Recall that because operations in a
single process do not overlap, this definition is unambiguous.
...
% When real-time constraints are not important, this provides
...
or backward in time, but they cannot overtake each other; they must
respect the original order of operations within that process, which
corresponds to respecting their program order. However, beads on
different strings---representing operations from different
processes---are free to slide past each other. Intuitively, this
flexibility allows the application to reorder events globally,
creating a consistent history as long as the operations from each
process still occur in the order they were issued.
...
operations in $P_2$ along their worldline so that they occur after the
operations in $P_1$, we arrive at a sequentially consistent history
where $P_1$ and $P_2$ read different values for $x$. By a similar
sliding, we can also produce a sequentially consistent history in
Figure \ref{fig:smEx3-b} where the second read operation in $P_2$
returns $4$ after the first one reads $5$, despite the fact that in
\emph{real time}, $\memwrite{x}{4}$ occurs \emph{before}
$\memwrite{x}{5}$.

Since external order is more strict than external order (meaning it
imposes more constraints in general), a linearizable system is always
sequentially consistent, though the converse is not true. Hence, the
executions in Figure \ref{fig:smEx2} are sequentially consistent.
\begin{lemma}
...
not linearizable---as noted earlier, the only linearizable executions
would constrain all three read operations to return the same value for
$x$, either $3$ or $5$.
...
(they run on distinct computers, after all). The programmer can only
be sure that each process will run its own operations in program
order. Therefore, before we run the program, the executions in
\ref{fig:smEx3-a} and \ref{fig:smEx3-b} are just as expected as the
execution in Figure \ref{fig:smEx2}.
...
%This sliding allow noe to construct a fairly arbitrary
...
and the real-world experience stems from a well-understood and
fundamental tradeoff between coherence and performance. The more
``coherence'' we demand from the system, the more the processes have
to communicate over the network, whose unpredicable delays impose
overheads that degrade performance. Conversely, the more we demand
immediate answers from our system, the less time a process has to
communicate with other processes, so the responses we get are not as
...
% Real conditions in our context are likely to be chaotic, featuring
% many smaller disruptions and delays and sometimes larger
...
% system handles requests at all. Altogether, the extremes of C, A,
% and P in the CAP theorem are not the appropriate conditions to apply
% to many, perhaps most, real-world applications.
