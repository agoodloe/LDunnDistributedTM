...
\begin{definition}(Causally ordered delivery)
  ...
  different senders), $m^\textrm{send} \to n^\textrm{send}$, then
  $m^\textrm{recv} \to n^\textrm{recv}$. That is, each destination
...
\begin{definition}[CO broadcast]
  ...
  $m$ and $n$, if $m^\textrm{send} \to n^\textrm{send}$, then all
  destinations deliver $m^\textrm{recv}$ before $n^\textrm{recv}$.
\end{definition}
