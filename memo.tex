...
separately.
\begin{gather}
  \EstUnseenPos{Q}{F}{P} \equiv \{ w \in \EstUnseen^{Q}{\left(P\right)} | \NumWeight\left(w, F\right) > 0 \} \\
  \EstUnseenNeg{Q}{F}{P} \equiv \{ w \in \EstUnseen^{Q}{\left(P\right)} | \NumWeight\left(w, F\right) < 0 \}
\end{gather}
...
be unseen by $P$.
\begin{gather}
  \twp{Q}{F}{P} \equiv \sum \{ \NumWeight\left(w, F\right) | w \in \EstUnseenPos{Q}{F}{P} \} \\
  \twn{Q}{F}{P} \equiv \sum \{ \NumWeight\left(w, F\right) | w \in \EstUnseenNeg{Q}{F}{P} \}
\end{gather}
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \Videal{F} - \conitAt{F}{P} =  \sum_{Q \in \AllProc \setminus \{P\}} \{ \NumWeight\left(w, F\right) | w \in \Unseen^Q\left(P\right) \}
  \end{equation*}
  Because actual unseen writes are a subset of estimated unseen writes, we have the following equation:
  \begin{align*}
    & \sum \{ \NumWeight\left(w, F\right) | w \in \Unseen^Q\left(P\right) \} \\
    < & \sum \{ \NumWeight\left(w, F\right) | w \in \EstUnseenPos{Q}{F}{P} \}\\
    \equiv & \ \twp{Q}{F}{P}
  \end{align*}
  Likewise we have the following:
  \begin{align*}
    &  \twn{Q}{F}{P} \\
    \equiv & \sum \{ \NumWeight\left(w, F\right) | w \in \EstUnseenNeg{Q}{F}{P} \}\\
    < & \sum \{ \NumWeight\left(w, F\right) | w \in \Unseen^Q\left(P\right) \}
  \end{align*}
...
acquires locks (i.e. exclusive rights to modify) covering any
of the data items affected by the write on $P$ and any other required
processes. After acquiring all locks, the write is executed against
$\Dstate{Q}$ and pushed to all destinations, and then returns to
the user after receiving all acknowledgments.
...
An indirect way this liability might have been prevented is if $Q$ had
enforced $\NumErr(Q, F_{\textrm{flight}}) = 0$. Indeed, if the
two-phase locking policy is used (see
...
\begin{comment}
However, why, suppose another reservation server $R$ is notified about
both $w_Q$ and $w_P$, so it has $0$ numerical error, but learns about
them in the opposite order and ends up in state
$D_R = \Dinit + w_Q + w_P$ where $Q$'s client appears to get the
reservation first. This state, which has $0$ numerical error, would
persist at $R$ until the total ordering component at $R$ discovers
that $w_P$ should be first. Numerical error only controls which
updates a process sees, but not the order in which they are applied to
the local replica.
\end{comment}
...
\begin{definition}[Order error]
  The \emph{order error} of an access $a$ depending on a conit set
  $\mathcal{F}$ is the sum of $\OrderWeight\left(a, F\right)$ for
  each $F$ in $\Conits$, for all writes $w$ that are in the out-of-order
  set for $\mathcal{F}$.
  \begin{equation}
    \OrderError\left(a, \mathcal{F}\right) \equiv \sum_{F \in \Conits} \{ \OrderWeight\left(w, F\right) | w \in \mathsf{OOO}\left(a, \Conits\right) \}
  \end{equation}
\end{definition}
...
following value:
\begin{equation}
  \EstOrderError\left(a, \mathcal{F}\right) \equiv \sum_{F \in \Conits} \{ \OrderWeight\left(w, F\right) | w \in \Uncommitted{P}|_\Conits \}
\end{equation}
Now
$\mathsf{OOO}\left(a, \Conits\right) \subseteq
\Uncommitted{P}|_\Conits$ implies
$\OrderError\left(a, \mathcal{F}\right) \leq \EstOrderError\left(a,
  \mathcal{F}\right)$. Therefore it suffices for $P$ to bound the
estimated order error of each access.
...
One can imagine two ways to bound out-of-order error, though which is
the intention of the authors is not entirely made clear in
\cite{2002tact}. First, $\OErr(P, F)$ can be taken as the allowable
upper bound of $\OrderError\left(a,\{F\}\right)$ for an access $a$,
where $\{F\}$ means the set consisting of just $F$. While handling
this access, for each conit $F$ that $a$ depends on, if
$\EstOrderError(a, \{F\})$ exceeds $\OErr(P, F)$, $P$ engages the
write commitment procedure to commit all its tentative updates, until
the order error $0$. This process is repeated for the entire set of
conits $a$ depends on.

Alternatively, the definition of $\mathsf{OOO}\left(a, \Conits\right)$
suggests considering conits as a group, rather than bounding their
order errors individually. With this policy, an access specifies the
entire set of conits it depends on as a set $\Conits$, and
additionally specifies its error bound for
$\OrderError\left(a,\Conits\right)$. Then, $P$ checks whether
$\EstOrderError(a, \Conits)$ exceeds this value, and engages write
commitment if necessary. Note that this measurement of out-of-order
error considers the relative order of all updates that affect conits
in $\Conits$, even ones that affect different conits, which differs
from the policy above where each conit is considered separately. Which
of these mechanisms, if either of them, was intended by Yu and
Vahdat is not totally clear to us.
...
\begin{example}
  ...
  committed. Figure~\ref{fig:conit-order-b} shows a possible state at
  a later time $t = 13$.
\end{example}
