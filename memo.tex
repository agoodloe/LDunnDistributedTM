...
tradeoffs. In the context of data replication, a central theme is a
general tradeoff between performance and consistency.
...
communication delays for coordination. These assumptions are properly
known as the asychronous network model.
...
send event $m^\textrm{send}$ indicating the moment the message is sent
over the network by its originating process; its delivery generates a
receive event $m^\textrm{recv}$. The (send, receive) pairs are
connected by an arrow and said to be corresponding events. The
...
the effect of network latencies on message delivery. Because they
arrive delayed, two messages may be delivered in a different order
...
conventional ways around the problem.
...
assumption all devices have synchronized clocks.
...
every event except $\mrecv{1}$, and all other events are totally ordered
by causality. Relations like $\sync{}{}$ that are reflexive and
symmetric but not necessarily transitive are sometimes called
\emph{compatibility relations}.
...
%$\msend{1} \to \msend{2}$, so the clock consistency
...
compare timestamps to infer events are causally related. A system of
scalar clocks, defined below, can enforce the weaker condition but not
the strong one.
...
\begin{definition}[Vector comparison]
  ...
  neither is greater than the other, we write $\sync{v}{w}$ and say the
...
\ref{fig:message-latencies-matrix} shows our running examples
alongside matrix clock timestamps.

Each process $P_i$ stores an $N$ by $N$ matrix $\vt_i$, initialized to
...
\item[\textbf{R2}] When a message is received from $P_j$ with a piggybacked timestamp $\vt$,
  ...
  \item Update all other rows $\vt_i[j', -]$ for $j'\neq j$ according
    to
...
timestamps less than or equal to $t$. Now with these assumptions,
$P_i$ can use the value $\vt_i[j, O]$ as a lower bound estimate of
which updates originating from $P_O$ $P_j$ has already been informed
about. In particular, once we have the condition
...
originating at $P_O$. In Section \ref{sec:continuous-consistency} we
will see an example where this sort of lower bound estimate of other
process's clocks is important for distributed database replication.

\subsection{Message Ordering and Group Communication}
We have seen that the network latencies tend to cause messages to be
delayed and arrive in a different order than they were sent in. In
this section we summarize some different assumptions we could impose
about how messages are ordered. Then we discuss generalizing our model
to allow sent to multiple receipients at once, such as in a group chat
application, where message ordering becomes a critical issue.

\subsubsection{Message Ordering}
We briefly explain some terminology before proceeding. In general,
applications should not show messages to the user immediately when
they come in from the network, precisely because the network tends to
deliver messages in unexpected orders. Thus, we may want to wait for
other messages to play ``catch up.'' Thus, we make a distinction
between message \emph{receipt} and \emph{delivery}. Middleware layer.
...
\begin{definition}[FIFO condition]
  ...
  $\msend{1} \msend{2} \implies
  \mrecv{1} \mrecv{2}$.
\end{definition}

FIFO ordering is simple to implement and often built into the
transport layer of a network protocol stack. For instance, although
the Internet Protocol (IPv4) by itself does not
provide FIFO semantics, the associated TCP protocol, which many
applications use, does provide FIFO. As TCP demonstrates, maintaining
a FIFO channel can be as simple as marking outgoing messages with
consecutive increasing numbers (e.g. $1,2,3\ldots$). If message $1$
arrives and then $3$ arrives, we infer that $2$ is lagging behind,
delivering $1$ to the user immediately but withholding delivery of $3$
until after $2$ is received and delivered. As far as the end user can
tell, the messages arrive in the right order.

The guarantees provided by FIFO are minimal because they only apply on
a per-link basis. The next ordering model enforces a partial order
across messages in the system.
...
\begin{definition}(Causally ordered delivery)
  \label{def:causalorder} That is, if the following property holds:
  \begin{quote}
...
In mathematical terms, if we consider the set of all messages sent to
some fixed destination $P_\mathrm{dest}$, the function mapping send
events to corresponding receive events must be monotonic with respect
to causal precedence. That is, all destinations $P_\mathrm{dest}$
observe messages arriving in an order respecting the causality of
their send events.

\subsubsection{Group Communication}
The importance of message ordering is particularly acute. The FIFO
model does not really apply in group communication.
...
\ref{fig:message-latencies-scalar-c}, because $\msend{1}$
causally precedes $\msend{2}$, we find
\[C(\msend{1}) = 1 < 5 = C(\msend{2}).\] Using this
...
paradigm called the \emph{distributed shared memory} (DSM)
framework.

In this setting, programmers think in terms of reading and writing
...
To implement the DSM model, a somewhat ``middleware'' layer called the
...
\item[Reads] A \emph{read} request $R(x)$ indicates reading the value
  ...
  important to indicate the return value is $v$, we write $R(x, v)$.
...
operation. Each operation begins at a time $\Op.s$ ($s$ for ``start'')
...
processes over the network. While executing a read request $R(x)$ it
...
that the programmer should expect each $R(x)$ operation to return
